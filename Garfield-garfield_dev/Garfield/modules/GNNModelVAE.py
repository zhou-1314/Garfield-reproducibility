...
class GNNModelVAE(GAE):
    ...
    def __init__(
        ...
        # Ablation toggles (default True -> original behavior). The instance/cluster
        # contrastive "lambda" is a *temperature*, not a linear weight, so these
        # terms cannot be ablated via lambda=0; set these flags instead.
    ...
    def forward(self, data_batch, decoder_type, augment_type):
        ...
        if decoder_type == "omics":
            ...
            output["z"] = z1
            # Expose both augmented views' encoder latents (pre-projection) so that
            # view-view mutual information can be measured in latent space without the
            # untrained-projector confound present in the no-contrastive setting.
            output["z1_latent"] = z1
            output["z2_latent"] = z2
            output["z_1"] = z_1
            output["z_2"] = z_2
